...
void main() async {
  ...
  // await NotificationService().init();      //old code
  await Alarm.init(); //new codde
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _autoScheduleReminders(List<Prescription> prescriptions) async {
    ...
    for (final prescription in prescriptions) {
      for (final timing in prescription.timings) {
        ...
        final alarmId = '${prescription.id}_${timing.id}'.hashCode & 0x7fffffff;
        ...
        // ---- due now (±5 min)
        if (diff.abs() <= dueSoonWindow) {
          print('[AlarmPkg] ${prescription.medicine.name} is DUE NOW ($label)');
  ...
  void _showReminderDialog(Prescription prescription) async {
    ...
    try {
      ...
      final DateTime fireTime = diff.inMinutes <= 1
      ...
      body: _isLoading
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  SvgPicture.asset('assets/empty.svg', height: 150),
